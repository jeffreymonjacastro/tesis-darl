...
def plot_numeric_drift(
    ...
    ax.hist(before.dropna(), label="Antes del drift", color="#1f77b4", **kw)
    ax.hist(after.dropna(),  label="Después del drift", color="#ff7f0e", **kw)
...
def plot_numeric_drift_grid(
    ...
    for i, col in enumerate(cols):
        ax = axes[i]
        ax.hist(df_before[col].dropna(), label="Antes",  color="#1f77b4", **kw)
        ax.hist(df_after[col].dropna(),  label="Después", color="#ff7f0e", **kw)
...
def plot_categorical_drift(
    ...
    fig, ax = plt.subplots(figsize=figsize)
    ax.barh(y + h / 2, pb.values, h, label="Antes del drift", color="#1f77b4", alpha=0.8)
    ax.barh(y - h / 2, pa.values, h, label="Después del drift", color="#ff7f0e", alpha=0.8)
    ax.set_yticks(y)
...
def plot_categorical_drift_grid(
    ...
    for col, meta in metas.items():
        ...
        if p_after is None:
            continue
        fig = plot_categorical_drift(meta.p0, p_after, col, top_k=top_k,
                                     figsize=figsize_per)
        figs.append(fig)
    return figs
