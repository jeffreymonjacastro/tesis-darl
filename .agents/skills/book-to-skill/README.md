...
<p align="center">
  <strong>Turn any technical book, document folder, or collection of sources into a unified Codex project-local skill — ready to study, reference, and use while you work.</strong>
</p>
...
**book-to-skill solves this by turning the book into a structured skill Codex can load on demand.**

Once installed, you ask Codex to use the generated skill for a topic, and it reads the right chapter file instead of loading the full source. No digging through PDFs. The book becomes part of your workflow.

---

## 📦 What it generates

Running `book-to-skill your-book.pdf` (or a folder, glob, or list of files) creates a full skill at `.agents/skills/<slug>/` in this repository:
...
After the skill is created, use it like any other Codex project-local skill:

```bash
Ask Codex to use `designing-data-intensive-apps`                  # load core mental models
Ask Codex to use `designing-data-intensive-apps` for replication   # find and explain a topic
Ask Codex to use `designing-data-intensive-apps` ch05              # dive into chapter 5
Ask Codex what chapters `designing-data-intensive-apps` has
```
