---
name: book-to-skill
description: "Converts books and documents (PDF, EPUB, DOCX, HTML, Markdown, plain text, RTF, MOBI/AZW with Calibre) into structured Codex skills, extracting frameworks, mental models, principles, techniques, and anti-patterns. Use when the user wants to study a document through Codex, apply an author's frameworks while working, or build a reusable local knowledge base from a file."
---
...
Books contain crystallized expertise: frameworks, principles, and techniques that took years to develop. This skill extracts that knowledge into a Codex project-local skill format that can be reused repeatedly.
...
**Action:** Run all steps below (Steps 09)
...
**Action:** Run Steps 03, then produce a structured extraction report (frameworks, principles, techniques found). Stop  do NOT generate skill files.
...
**Action:** Skip Steps 03, use the provided analysis as input, run Steps 49
...
This converter can run from multiple skill systems. In this repository, when looking for this converter's helper script or writing the generated book skill, prefer these locations in order:

1. Codex project-local skills: `.agents/skills/`
2. Codex user skills: `%USERPROFILE%\.codex\skills`
3. Amp global skills: `~/.config/agents/skills/`
4. Amp legacy global skills: `~/.config/amp/skills/`

Generated skills should default to `.agents/skills/` when the current workspace contains that directory.
...
- `<tempdir>/book_skill_work/markdown_ast.json`  a hierarchical Markdown AST, source-line ranges, outlines, and structural warnings. This file is created only when at least one `.md` or `.markdown` source is parsed successfully.

Markdown sources are parsed with `markdown-it-py` using CommonMark plus tables. Their original text is preserved verbatim for `full_text.txt`; parsing does not re-render or normalize it. If the parser dependency is unavailable or parsing fails, extraction continues with the raw Markdown text and records the fallback warning in `metadata.json`.
...
For books over ~50k tokens, prefer programmatic probes over `Read(full_text.txt)` without bounds. For Markdown inputs, use the compact per-source `outline` in `metadata.json` first; do not load the full `markdown_ast.json` unless the task needs structural details such as nested lists, tables, links, emphasis, or fenced code:
...
For Markdown sources with `ast_available: true`, read each source's `outline` from `metadata.json` first to map headings and their hierarchy. Consult `markdown_ast.json` only when heading metadata is insufficient or when relationships among lists, tables, links, emphasis, and fenced code matter. Line ranges in the AST are one-based and inclusive.

Use `full_text.txt` as the canonical source for synthesis and token estimates. Read its first 8,000 characters to identify:
- Book **title** and **author(s)**
- **Chapter structure** not already established by a Markdown outline (look for "Chapter N", "PART I", numbered headings, table of contents)
...
- **Codex project-local default**: `.agents/skills`
- **Codex user**: `%USERPROFILE%\.codex\skills` if the user explicitly wants a user-global skill
...
- Each chapter summary file: **8001,200 tokens** (dense, not verbose)
...
(510 most important terms from this chapter)

## Mental Models
<24 frameworks or thinking tools. Write as "Use X when Y" or "Think of X as Y">
...
(37 takeaways a practitioner must remember)
...
---
name: <skill_name>
description: "Knowledge base from \"<Full Title>\" by <Author(s)>. Use when applying <author>'s frameworks for <key topics, 36 terms>, studying the book, or referencing its concepts."
---
