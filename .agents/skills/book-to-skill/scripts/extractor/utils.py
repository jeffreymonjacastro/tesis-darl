...
def detect_structure(text: str) -> dict:
    ...
    chapter_pattern = re.compile(
        r"^\s*(chapter\s+\d+|CHAPTER\s+\d+|ch\.\s*\d+|\d+\.\s+[A-Z])", re.IGNORECASE
    )
    ...
    while i < len(args):
        ...
        if arg == "--mode":
            ...
            if i + 1 < len(args) and not args[i + 1].startswith("--"):
                ...
                i += 1
        elif arg == "--no-install-missing" or arg.startswith("-"):
            i += 1
        else:
...
def extract_single_file(
    ...
    if ext == ".epub":
        ...
        print(f"Extracting Markdown: {input_str}")
        text = read_text_file(input_str)
        if text is None or not text.strip():
            raise ExtractionError(
                f"Could not read Markdown document: {input_path.name}"
            )

        try:
            markdown_result = parse_markdown(text)
        except Exception as exc:  # noqa: BLE001 - any parser failure must preserve raw text.
            warning = (
                f"Markdown AST unavailable ({type(exc).__name__}: {exc}); "
                "raw Markdown text was preserved."
            )
            print(f"WARNING: {warning}", file=sys.stderr)
            markdown_warnings.append(warning)
            method = "plain-text"
        else:
            markdown_ast = markdown_result["ast"]
            markdown_outline = markdown_result["outline"]
            markdown_warnings = markdown_result["warnings"]
            markdown_parser = markdown_result["parser"]
            markdown_parser_version = markdown_result["parser_version"]
            method = "markdown-it-py"
    ...
    if markdown_ast is not None:
        raw_structure = detect_structure(text)
        structure = {
            "chapters_detected": len(markdown_outline),
            "chapter_headings_sample": [
                heading["title"] for heading in markdown_outline[:10]
            ],
            "has_toc": raw_structure["has_toc"],
        }
    else:
...
def main():
    ...
    # Build the optional Markdown AST artifact before metadata references it.
    markdown_sources = [
        source for source in extracted_sources if source["format"] in {"md", "markdown"}
    ]
    ast_available = any(
        source["markdown_ast"] is not None for source in markdown_sources
    )
    output_ast = None
    if ast_available:
        parser_version = next(
            source["markdown_parser_version"]
            for source in markdown_sources
            if source["markdown_parser_version"] is not None
        )
        ast_payload = {
            "schema_version": "1.0",
            "parser": "markdown-it-py",
            "parser_version": parser_version,
            "line_map_convention": "one-based inclusive",
            "sources": [
                {
                    ...
                    "ast": source["markdown_ast"],
                }
                for source in markdown_sources
            ],
        }
        OUTPUT_AST.write_text(
            json.dumps(ast_payload, indent=2, ensure_ascii=False),
            encoding="utf-8",
        )
        output_ast = str(OUTPUT_AST)
    else:
        OUTPUT_AST.unlink(missing_ok=True)
    ...
    # Consolidate the per-source structure so Markdown outlines remain authoritative.
    consolidated_structure = {
        "chapters_detected": sum(
            source["chapters_detected"] for source in extracted_sources
        ),
        "chapter_headings_sample": [
            heading
            for source in extracted_sources
            for heading in source["chapter_headings_sample"]
        ][:10],
        "has_toc": any(source["has_toc"] for source in extracted_sources),
    }
    ...
    metadata = {
        ...
        "filename": "multi-source"
        if len(extracted_sources) > 1
        else extracted_sources[0]["filename"],
        "format": "mixed"
        if len(extracted_sources) > 1
        else extracted_sources[0]["format"],
        ...
        "markdown_source_count": len(markdown_sources),
        "markdown_ast_source_count": sum(
            source["markdown_ast"] is not None for source in markdown_sources
        ),
        "sources": [
            {
                ...
                "has_toc": src["has_toc"],
                "ast_available": src["markdown_ast"] is not None,
                "outline": src["outline"],
