...
\begin{enumerate}
    ...
    recompensa acumulada esperada sobre los cuatro conjuntos de datos del
    \textit{benchmark} TableShift \cite{gardner2023tableshift}: ICU Hospital Mortality,
    Hospital Readmission, Income y Food Stamps.
...
datos tabulares en modalidad \textit{batch}. La evaluación empírica se realiza sobre cuatro
conjuntos de datos del \textit{benchmark} TableShift: ICU Hospital Mortality, Hospital
Readmission, Income y Food Stamps \cite{gardner2023tableshift}. El \textit{distribution shift} se
...
El agente de aprendizaje por refuerzo se entrena utilizando el algoritmo PPO implementado
sobre la librería Stable-Baselines3, con un entorno de simulación construido bajo la
interfaz estándar de Gymnasium. El entrenamiento se realiza exclusivamente sobre episodios
de drift sintético generados a partir de los cuatro datasets seleccionados; no se contempla
...
continuo. La evaluación se restringe a los cuatro \textit{datasets} seleccionados y a un
